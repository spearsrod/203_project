...
def find_nearest_path(pos_cur, path, env):
	distance = np.sqrt(np.square(pos_cur[0] - path[:,0]) + np.square(pos_cur[1] - path[:,1]))
	idx = np.argmin(distance)

	angle_to_path = np.arctan2(path[idx,1] - pos_cur[1], path[idx,0] - pos_cur[0])
	dif_angle = angle_to_path - pos_cur[2]
	sign = np.sign(np.sin(dif_angle))

	cur_angle = pos_cur[2]
	init_angle = env.car.init_angle

	dif_init = init_angle - cur_angle
	if(np.cos(dif_init) < 0):
		sign = -sign
	
	# sign = -np.sign(pos_cur[1] - path[idx,0])
	# print('check sign here')
	# print(sign)
...
def lookahead_controller(env, path, path_state, car_state, speed_profile, path_k):
	car = env.car
	Kla = 3500
	xla = 15
	...
	e = path_state[1]
	d_psi = path_state[2]
	path_idx, e = find_nearest_path(car_state, path, env)
	...
	Caf = 275000
	Car = 265000
	understeer = car.m*(car.hull.a*Caf - car.hull.b*Car)/(car.hull.L*Caf*Car)
	dpsi_ss = path_k[path_idx]*(car.m*car.hull.a*Ux**2/(car.hull.L*Car) - car.hull.b)
	delta_ff = Kla*xla/Caf*dpsi_ss + path_k[path_idx]*(car.hull.L + understeer*Ux**2)
	delta = -Kla*(e + xla*d_psi)/Caf + delta_ff
	return -delta, Fxtotal

def segment_track(path_s, path_k):
	new_k = np.zeros(path_k.size)
	for idx in range(0, path_k.size):
		if(np.abs(path_k[idx]) < 1.0/np.power(10, 6)):
			new_k[idx] = 0
		else:
			new_k[idx] = (1.0/3.0)*(np.sum(path_k[idx:idx + 2]))

def generate_speed_profile(path_s, path_k, env):

	segment_track(path_s, path_k)
	...
	for idx in range(0,path_k.size):
		...
		profile[idx] = np.minimum(max_Ux*0.5, 40)
	return profile
...
def main():
	...
	for idx in range(1000):
		...
		delta, Fx = lookahead_controller(env, interp_center_line, path_state, car_state, speed_profile, interp_k)
		if(Fx > 0):
			action = [delta, Fx, 0]
		else:
			action = [delta, 0, -Fx]
